...
EDA란 탐색적 자료 분석, 즉 **데이터가 어떤 자료로 구성되어있고 분포는 어떠한지를 보는 것**입니다. 
...
`한가지 잊지 말아야 할것은 우리가 가진 데이터는 sample 데이터이므로 우리는 sample 데이터를 통해 단지 population 분포와 parameter를 파악한다는 것에 주목해야합니다.`
...
바로 데이터의 평균이나 분산 등과 같은 정확한 값이 필요할 때 입니다.
...
샘플데이터가 모집단의 변수를 잘 나타낼 수도 있고 그러지 않을 수도 있는데 중요한 것은 여기서 우리가 해야될 일은 바로
...
> 위에서 분포를 평가하기 위헤 구한 값은 categorical variable에서는 아무 의미가 없습니다.
...
![Find Skew or Kurtosis through estimator](/greenjun/_post/assets/images/skewnesskurtosis.png)
/okjsp/okjsp-site/raw/images/okjsp_logo.gif
